...
<template>
  <div class="w-80 bg-netflix-gray-23">
    ...
    <div class="flex justify-between text-white/70 mb-2">
      <div class="text-lg" data-testid="movie-card-title">{{ title }}</div>
      <div
        class="text-sm border border-netflix-gray-97/50 rounded px-4 py-0.5 h-7"
        data-testid="movie-card-year"
      >
        {{ year }}
      </div>
    </div>
    <div class="text-white/50 text-sm" data-testid="movie-card-genres">
      {{ genres.join(' & ') }}
    </div>
  </div>
</template>
